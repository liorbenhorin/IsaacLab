...
class RenderData:
    ...
    def _build_distortion_rays(self) -> wp.array(dtype=wp.vec3f, ndim=4):
        ...
        Inverts the OpenCV forward distortion model per output pixel to recover the undistorted ray
        direction, honoring the calibrated ``fx/fy/cx/cy`` (non-square, off-center) intrinsics. When
        :attr:`OpenCvDistortionCfg.apply_lens_distortion` is ``False`` the coefficients are treated as
        zero, so the rays are undistorted but still respect the off-center principal point and
        non-square focal lengths (matching how the RTX/OVRTX path mutes the coefficients while keeping
        the intrinsics).
        """
        from .opencv_distortion_rays import compute_camera_rays_opencv_fisheye, compute_camera_rays_opencv_pinhole
