...
"""Kit-less, GPU-less tests for the OpenCV lens-distortion ray-generation kernels.

These exercise the warp kernels in
:mod:`isaaclab_newton.renderers.opencv_distortion_rays` on the warp CPU device, without ``newton``,
a renderer or a GPU. The kernels invert the OpenCV forward distortion model to recover the
...
if not _MISSING_MODULES:
    import warp as wp
    from isaaclab_newton.renderers.opencv_distortion_rays import (
        compute_camera_rays_opencv_fisheye,
...
_FISHEYE_COEFFS = dict(k1=0.1, k2=-0.02, k3=0.0, k4=0.0)
...
def _launch_fisheye(coeffs: dict) -> np.ndarray:
    """Launch the fisheye kernel on the warp CPU device and return the ray field as NumPy."""
    rays = wp.empty((1, HEIGHT, WIDTH, 2), dtype=wp.vec3f, device="cpu")
    wp.launch(
        compute_camera_rays_opencv_fisheye,
        dim=(1, HEIGHT, WIDTH),
...
def test_fisheye_ray_origins_are_zero_and_directions_unit():
    """Every fisheye ray has a zero origin and a unit-length direction looking down -Z."""
    rays = _launch_fisheye(_FISHEYE_COEFFS)
    origins = rays[..., 0, :]
    directions = rays[..., 1, :]
    assert np.allclose(origins, 0.0)
    norms = np.linalg.norm(directions, axis=-1)
...
def test_fisheye_inversion_round_trips_incidence_angle():
    """The recovered ray's incidence angle reproduces the OpenCV equidistant forward model per pixel."""
    c = _FISHEYE_COEFFS
    rays = _launch_fisheye(c)
    directions = rays[0, :, :, 1, :]
    max_err = 0.0
    for py in range(0, HEIGHT, 7):
        for px in range(0, WIDTH, 9):
            x_d, y_d = _pixel_distorted_normalized(px, py)
            theta_d = math.hypot(x_d, y_d)
            # recover the incidence angle theta from the ray direction (angle off the -Z axis)
            direction = directions[py, px]
            theta = math.acos(min(1.0, -float(direction[2])))
            t2 = theta * theta
            theta_d_fwd = theta * (1.0 + c["k1"] * t2 + c["k2"] * t2**2 + c["k3"] * t2**3 + c["k4"] * t2**4)
            max_err = max(max_err, abs(theta_d_fwd - theta_d))
    assert max_err < 1e-5, f"fisheye inversion round-trip error {max_err:.2e} too large"


def test_fisheye_zero_coeffs_is_equidistant_identity():
    """With zero coefficients theta_d == theta, so the recovered radius is tan(theta_d) along the pixel."""
    zero = dict(k1=0.0, k2=0.0, k3=0.0, k4=0.0)
    rays = _launch_fisheye(zero)
    directions = rays[0, :, :, 1, :]
    for py in range(0, HEIGHT, 11):
        for px in range(0, WIDTH, 13):
            x_d, y_d = _pixel_distorted_normalized(px, py)
            ...
            direction = directions[py, px]
            theta = math.acos(min(1.0, -float(direction[2])))
            assert theta == pytest.approx(theta_d, abs=1e-5)
